...
overallCooldowns = convertspellids(overallCooldowns)
...
local function CreateIcon(anchor)
	...
	icon.isNeedStart = function(sentCD)
		cooldown = tonumber(sentCD);
		activeCooldown = icon.cooldown
		endTimeCooldown = icon.starttime + activeCooldown
...
function PAB:UpdateAnchors(updateIcons)
	for i=1,GetNumPartyMembers() do
		...
		for ability,cooldown in pairs(db.abilities[anchor.race]) do
			local enabled = db.enabledCooldowns[anchor.race][ability]
			if enabled then
...
function PAB:CheckAbility(anchor,ability,cooldown,pIndex)
	...
	for index,icon in ipairs(anchor.icons) do
		-- Direct cooldown
